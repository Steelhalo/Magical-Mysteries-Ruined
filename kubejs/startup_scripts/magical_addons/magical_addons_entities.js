...
StartupEvents.registry('entity_type', event => {
    ...
    .onHitBlock(context => {
        ...
        entity.getLevel().addParticle('minecraft:end_rod', entity.getX(), entity.getY(), entity.getZ(), 0, 0.2, 0);
    })
    .onHitEntity(context => {
        const { entity, result } = context;
...
StartupEvents.registry('entity_type', event => {
    event.create('arrow', 'entityjs:arrow')
        ...
        .mobCategory('misc')
        .item(item => {
            item.maxStackSize(63);
            item.texture('magical_addons:item/arrow');
        })
        .textureLocation(entity => {
            return "magical_addons:textures/entity/projectiles/arrow.png"
        })
        .canHitEntity(entity => {
            return entity.type == "minecraft:zombie";
        })
        ...
        .onHitBlock(context => {
            const { entity, result } = context;
            entity.getLevel().addParticle('minecraft:flash', entity.getX(), entity.getY(), entity.getZ(), 0, 0.025, 0);
        })
        .onHitEntity(context => {
            const { entity, result } = context;
            if (result.entity.living) {
                let potion = result.entity.potionEffects
                potion.add('minecraft:glowing', 100, 1, false, true)
            }
        })
        .tick(entity => {
